...
Backup-ClientHealtLog # zip and remove old logs to keep log folder clean
...
$ClientHealthTaskName = 'ConfigMgr Client Health -'
...
If ($null -eq $Log.ClientInstalledReason -and ("$($Log.ClientVersion)" -ne '') -and (([version]$log.ClientVersion) -ge $MinimumClientVersion)) {

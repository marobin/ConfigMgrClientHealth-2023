...
Function Get-Info {
    New-Object PSObject -Property @{
        Hostname         = $WMIComputerSystem.Name;
...
Function Get-OperatingSystemFullName {
    "$($WMIOperatingSystem.OSName) $($WMIOperatingSystem.BuildVersion)"
}
...
Function Search-CMLogFile {
    ...
    :loop for ($i = ($LogData.Count - 1); $i -ge 0; $i--) {
        ...
        try {
            ...
            $Error.RemoveAt(0)
            Write-Log -Message "Could not parse the line $($i) in '$($LogFile)': $($LogData[$i]) : $ErrorMessage" -Type 'WARNING'
            continue
        }
...
Function Backup-ClientHealtLog {
    [CmdletBinding()]
    Param ( )
...
Function Test-BITS {
    ...
    if ($BitsCheckEnabled -eq $true) {
        $BitsErrorList = Get-BitsTransfer -AllUsers | Where-Object { ($_.JobState -like "TransientError") -or ($_.JobState -like "Transient_Error") -or ($_.JobState -like "Error") }
...
Function Test-ConfigMgrClient {
    ...
    if (Get-Service -Name ccmexec -ErrorAction SilentlyContinue) {
        ...
        If (("$($Log.WMI)" -ne '') -and ($Log.WMI -ne 'OK')) {
            $Reinstall = $true
            $Uninstall = $true
        }
...
Function Invoke-SCCMClientCleanup {
    ...
    $ItemsToDelete = @(
        ...
        "$env:SystemRoot\ProgramData\Microsoft\Crypto\RSA\MachineKeys\19c5cf9*", # https://github.com/sntcz/Clear-MachineKeys # https://narkiewicz.ovh/kb/sccm-client-installation-error-codes/
        "$env:SystemRoot\CCM"
    )
...
Function Remove-CCMOrphanedCache {
    ...
    try {
        ...
        $AllCachedFolders = (Get-ChildItem -Path $CCMCache -Force -ErrorAction SilentlyContinue) | Select-Object Fullname -ExpandProperty Fullname
...
Function Start-RebootApplication {
    ...
    $task = schtasks.exe /query | FIND /I $taskName
    if (($null -eq $task) -or ($task -match '^\s*$')) { 
        New-RebootTask -taskName $taskName 
    }

    $null = Invoke-Executable -FilePath 'schtasks.exe' -ArgumentList "/Run /TN $taskName"
    Write-Log -Message "Ran task '$taskName' : $RunTaskLog"
}
...
Function New-RebootTask {
    ...
    $null = Invoke-Executable -FilePath 'schtasks.exe' -ArgumentList "/Create /tn $taskName /tr `"$execute $argument`" /ru `"BUILTIN\Users`" /sc ONCE /st 00:00 /sd 01/01/1901"
    #}
    <#
        else {
            $action = New-ScheduledTaskAction -Execute $execute -Argument $argument
            $userPrincipal = New-ScheduledTaskPrincipal -GroupId "S-1-5-32-545"
            $null = Register-ScheduledTask -Action $action -TaskName $taskName -Principal $userPrincipal
        }
        #>
}
...
Function Test-Service {
    ...
    elseif ($StartupType -like "Automatic (Delayed Start)") {
        ...
        if ( (($name -eq "wuauserv") -or ($name -eq "W32Time")) -and ($OSName -match "Windows (10|11)|Server 2016|Server 2019") ) {
            ...
            $null = Invoke-Executable -FilePath 'sc.exe' -ArgumentList "config $service start= delayed-auto"
    ...
    if ($service.Status -eq "Running") {
        ...
        try {
            ...
            If ($_.Exception.Hresult -eq '-2146233087') {
                ...
                $null = Invoke-Executable -FilePath "$env:ComSpec" -ArgumentList "/c sc config $Name type= own"
...
Function Test-RegistryPol {
    ...
    if ($Days -and (Test-Path -Path $MachineRegistryFile)) {
        Write-Log -Message "Check machine registry file to see if it's older than $($Days) days."
        try {
            $fileDate = Get-ChildItem -Path $MachineRegistryFile -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty LastWriteTime
            If ($null -ne $fileDate) {
                $regPolDate = $fileDate
            }
            Else {
    ...
    if ($RepairReason -ne "") {
        ...
        Write-Log -Message "GPO Cache: Broken ($RepairReason). Deleting registry.pol and running gpupdate..." -Type 'WARNING'

        try { if (Test-Path -Path $MachineRegistryFile) { Remove-Item $MachineRegistryFile -Force } }
        catch { Write-Log -Message "GPO Cache: Failed to remove the registry file ($($MachineRegistryFile))." -Type 'WARNING' }
        finally { 
...
Function Get-LocalFilesPath {
    ...
    if ($null -eq $obj) { $obj = Join-Path -Path $env:SystemDrive -ChildPath "ClientHealth" }
    Return $obj
}
...
function Register-DLLFile {
    ...
    Foreach ($Path in $FilePath) {
        ...
        $null = Invoke-Executable @DLLSplat
        Write-Log -Message "Register DLL '$Path'"
    }
}

...
|Pending Reboot |`PendingReboot`|Redémarrage en attente | OK</br>Pending Reboot|
|Last Boot Time |`LastBootTime`|Dernier redémarrage ||
|Sitecode |`Sitecode`|Code site actuel| |
|Domain|`Domain`|Domaine lié à la machine||
|WMI |`WMI`|Etat de la base WMI| OK</br>Repaired</br>Corrupt</br>PolicyPlatform Recompiled.|
|DNS |`DNS`|Etat des enregistrements DNS| OK</br><DNS IP>|
|Client Certificate |`ClientCertificate`|Etat des certificats du client| OK</br>Failed to find the certificate in the store</br>[RegTask] - Server rejected registration 3 |
|Provisioning Mode |`ProvisioningMode`|Mode de provisionnement| OK</br>Repaired|
|Drivers |`Drivers`|Etat des pilotes| OK</br><number> unknown or faulty driver(s)|
...
|WUAHandler |`WUAHandler`|Etat du composant de mise à jour| OK</br>Checking</br>Broken (WUAHandler Log)</br>Broken (File Age)</br>Broken (Event Log)</br>Repaired (WUAHandler Log)</br>Repaired (File Age)</br>Repaired (Event Log) |
|Admin Share |`AdminShare`|Etat des partages administratifs| OK</br>Repaired|
|Services |`Services`|Etat des services| OK</br>Started</br>Restarted|
|BITS |`BITS`|Etat du service BITS| OK</br>Remediated</br>ERROR</br>PS Module BitsTransfer missing|
|State Messages |`StateMessages`|Etat des messages d'état| OK</br>Repaired|
